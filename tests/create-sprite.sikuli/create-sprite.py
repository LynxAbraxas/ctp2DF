    ...
    if waitVanish(Pattern("ctp2progress-bar.png").similar(0.95), 100): # wait until progressbar vanishes
        ...
        for spriteNumber in sys.argv[1:]:
            ...
            while not success and tries < 4:
                ...
                print("Processing " + spriteFName + " try: ", tries)
                ...
                fpath= os.path.abspath("/opt/ctp2/ctp2_data/default/graphics/sprites/" + spriteFName + '_.spr')
                success= os.path.isfile(fpath) and os.path.getsize(fpath) > 0
                tries+= 1
    else:
        exit(20)
    exit(0)
